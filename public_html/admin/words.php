          ...
          <div class="card mb-3">
            <div class="card-body">
              <div class="row" id="main-body-div">
                <?php
                  if (empty($searchResult) === false) {
                    foreach ($searchResult as $product) {
                      echo('<div class="mb-4 col-md-6 col-lg-4">');
                        echo('<div class="border rounded-1 h-100 d-flex flex-column justify-content-between pb-3 rank_item_img_box">');
                          echo('<div class="overflow-hidden">');
                            echo('<div class="position-relative rounded-top overflow-hidden">');
                              echo('<a class="d-block" href="../admin/starter.php?word=' . $product["asin"] . '" onclick="sendPostDetail(event);" data-asin="' . $product["asin"] . '" >');
                                echo('<img class="img-fluid rounded-top rank_item_img" src="https://m.media-amazon.com/images/I/' . $product["image"] . '" alt="' . $product["name"] . '"  data-asin="' . $product["asin"] . '"/>');
                              echo('</a>');
                            echo('</div>');
                            echo('<div class="p-3">');
                              echo('<h5 class="fs-0">');
                                echo('<a class="text-dark" href="../admin/starter.php?word=' . $product["asin"] . '" onclick="sendPostDetail(event);" data-asin="' . $product["asin"] . '" >');
                                  echo($product["name"]);
                                echo('</a>');
                              echo('</h5>');
                              echo('<br />');
                              echo('<div style="text-align:left; color: #0B1727; font-size: smaller;">');
                            echo('カテゴリー： ' . $product["category"] . '<br />');
                            echo('ASIN： ' . $product["asin"] . '<br />');
                            echo('メーカー： ' . $product["manufacturer"] . '</div>');
                            echo('型番：' . $product["model"] . '<br />');
                            echo('</div>');
                          echo('</div>');
                          echo('<div class="d-flex flex-between-center px-3">');
                            echo('<div style="margin-left: auto;">');
                              echo('<a class="btn btn-sm btn-falcon-default me-2" href="#!" data-bs-toggle="tooltip" data-bs-placement="top" title="Add to Wish List">');
